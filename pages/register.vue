<template>
    <div class="scrollHide h-screen w-full flex">
        <div class="w-5/12 h-full bg-blue-700">
          <div class="h-full w-full flex items-center justify-center transition ease-in-out" v-if="cadence === 1">
            <img src="/img/undraw_seeker.png" />
          </div>
          <div class="h-full w-full flex items-center justify-center transition ease-in-out" v-if="cadence === 2">
            <img src="/img/undraw_recruiter.png" />
          </div>
        </div>
        <div class="w-7/12 h-screen flex items-center justify-center">
            <div class="flex flex-col items-center justify-center gap-4">
            <Logo />
            <p class="w-1/2 text-center">Unlock Success with Our
            Streamlined Connections and Endless Opportunities.
            Your Path to Career Excellence Starts Here!</p>
            <div class="w-full">
            <div class=" w-1/2 px-2 py-2 mx-auto rounded-lg bg-[#E3E3E3] flex text-base font-normal font-montserrat">
                        <div
                        :class="` transition duration-500 ${cadence === 1 ? 'font-semibold bg-blue-700' : 'hover:bg-[#E3E3E3]'} rounded-lg cursor-pointer w-1/2 text-center py-px md:py-2`"  @click="cadence = 1">
                            <p class="text-xxs md:text-base text-white">JOB SEEKERS</p>
                        </div>
                        <div class="p-1"></div>
                        <div :class="` transition duration-500 flex items-center justify-center ${cadence === 2 ? 'font-semibold bg-blue-700' : 'hover:bg-[#E3E3E3]'} rounded-lg cursor-pointer w-1/2 text-center py-px md:py-2`"  @click="cadence = 2">
                            <p class="text-xxs md:text-base text-white">RECRUITERS</p>
                            
                        </div>
                    </div>
                    <div class="flex flex-col items-center justify-center gap-3 p-6">
          <div class="flex flex-col">
            <span class="font-semibold">Username</span>
          <input
            placeholder=""
            type="text"
            class="focus:outline-none bg-grey px-16 py-3 rounded"
          />
          </div>
          <div class="flex flex-col">
            <span class="font-semibold">Email</span>
          <input
            placeholder=""
            type="text"
            class="focus:outline-none bg-grey px-16 py-3 rounded"
          />
          </div>
          <div class="flex flex-col">
            <span class="font-semibold">Password</span>
          <input
            placeholder=""
            type="text"
            class="focus:outline-none bg-grey px-16 py-3 rounded"
          />
          </div>
          <button class="focus:outline-none font-bold bg-blue-700 text-white px-14 py-2 rounded">
            SIGN UP
          </button>
        </div>
          </div>
        </div>
        </div>
    </div>
</template>

<script setup lang="ts">
    const cadence=ref(1);
</script>